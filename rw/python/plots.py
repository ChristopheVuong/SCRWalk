...
def plot_2dim_complex(
    ...
    """Plot 2-simplicial complex Gudhi style (can be easily extended with 3DCollection) along
    ...
    edges = np.array([s[0] for s in cplx.get_skeleton(1) if len(s[0]) == 2])
...
def plot_2dim_complexes(
    ...
    """Plot several 2-simplicial complex Gudhi style along
    with animation of paths if indicated
    Use Matplotlib that may be slow for this task"""
    # We need to set the plot limits.
    fig, axs = plt.subplots(nrows=1, ncols=len(l_cplxes))
    for i in range(len(l_cplxes)):
        axs[i].set_xlim(
        ...
        # list of triangles (point indices)
        triangles = np.array(
            [s[0] for s in l_cplxes[i].get_skeleton(2) if len(s[0]) == 3]
        )
        # list of edges (point coordinates)
        edges = np.array([s[0] for s in l_cplxes[i].get_skeleton(1) if len(s[0]) == 2])
